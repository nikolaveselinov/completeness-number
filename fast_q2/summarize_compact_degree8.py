...
"""Audit compact q=2, degree-eight archives and write family artifacts."""
...
def main() -> None:
    ...
    for path in files:
        ...
        if record.get("archive_kind") != ARCHIVE_KIND_DEGREE8:
            raise ValueError(f"{path} is not a degree-eight compact archive")
    ...
    write_csv(directory / "characteristics.csv", list(characteristics[0]), characteristics)
    ...
    lines = [
        "# Exact compact q=2, degree-eight completeness data",
        "",
        f"Validated characteristics: **{len(records)}/30**.",
        "",
        "Every archive rebuilds the supersingular context and exact 2,537-level "
        "universe. Its 3,655 unordered-pair certificates cover all 9,272,735 "
        "pair-level decisions by exhaustive Hom traversal or a successful "
        "all-target witness stop. All weights equal one.",
        "",
        "## E(p) distribution",
        "",
        "| E(p) | characteristics |",
    ...
    lines.extend(
        [
            "",
            "The scan is conclusive below U(p)=15; Theorem 1.2 proves every "
            "prime level of degree at least 15 is complete.",
            "",
        ]
    )
