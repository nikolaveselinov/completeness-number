...
"""Compute one or all compact exact q=2, degree-seven archives."""
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/local/q2_degree7"),
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main() -> None:
    ...
    characteristics = monic_irreducibles(A, 7)
    if len(characteristics) != 18:
        raise ArithmeticError(
            f"expected 18 degree-seven characteristics, got {len(characteristics)}"
        )
    index = args.index
    ...
    if index is not None:
        if not 0 <= index < len(characteristics):
            raise IndexError(f"index {index} is outside 0..17")
    ...
    for p in characteristics:
        ...
        result = compute_compact_q2_degree7(
            ctx,
